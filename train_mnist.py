...
class Dataset(torch.utils.data.Dataset):  # 与上面的方式均可以运行,因为最主要是需要有__getitem__() 与__len__()
...
def create_iterator(data, label, batchsize, shuffle=False):  #其实是创建一个自定义的dataset
    digit_indices = [np.where(label == i)[0] for i in range(10)]  # np.where()返回的是索引下标,包括每个维度上的索引,这里取第0个,否则返回的是(索引,)这样的索引
    # digit_indices 是标签为0,1,2,3,...的下标索引,分成10组
...
def main():
    ...
    train = dsets.MNIST(
        ...
        #  ]),  #因为后面需要转成ndarray来训练,所以不需要先转成tensor
        download=True
    )
    test = dsets.MNIST(
        root='../data/',
    ...
    def train(epoch):
        ...
        for batch_idx, (x0, x1, labels) in enumerate(train_loader):
           ...
           # train_loss.append(loss.data[0])  # 新版本改成loss.item()
            ...
            accuracy = []
            result = torch.abs(output1-output2)  # 计算两个输出点之间的绝对值距离
            distance = result.sum(dim =1)
            bool_ds = distance < 1.2  # 当输出点的距离小于一定的值,我们认为就属于同一类,而这个阈值,只能暂时通过实验得到一个值
            corrects = (bool_ds.type(torch.LongTensor) == labels.long().data).sum(dim = 0)
            accu = float(corrects)/float(labels.shape[0])
            accuracy.append(accu)

            # for idx, logit in enumerate([output1, output2]):  #这个原来的代码逻辑是有问题的,这样计算,accu始终在50左右,显然不对
